...
def main():
    ...
    folder_GT = 'D:/SR-Mamba/code/results/RGT/NWPU/19-49-08'  # GT 图像路径（不要包含中文）

    save_logger_folder = folder_GT
    ...
    img_list = sorted(glob.glob(osp.join(folder_GT, '*.png')))  # 真值图像路径列表

    for i, img_path in enumerate(img_list):
        base_name = os.path.splitext(os.path.basename(img_path))[0]
        print(base_name, img_path, os.path.join(folder_GT, base_name + suffix + '.png'))
        ...
        PSNR = AG_function(cropped_GT)

        # cropped_Gen = bgr2ycbcr(cropped_Gen)
        SSIM = niqe(cropped_GT)

        # print('{:3d} - {:25}. \tPSNR: {:.6f} dB, \tSSIM: {:.6f}'.format(
        #     i + 1, base_name, PSNR, SSIM))
        logger.info('Folder {} {:3d} - {:25}. \tAG: {:.6f} , \tNIQE: {:.6f}'.format(base_name,
            i + 1, base_name, PSNR, SSIM))
    ...
    logger.info('Folder {}-Average: AG: {:.6f} , NIQE: {:.6f}'.format(1,
        sum(PSNR_all) / len(PSNR_all),
        sum(SSIM_all) / len(SSIM_all)))
