...
def main():
    # Configurations
    crop = False  # 是否裁边
    crop_border = 8  # 左右裁剪4个像素,256*256变为248*248
    suffix = ''  # suffix for Gen images
    test_Y = True  # True: test Y channel only; False: test RGB channels

    # GT - Ground-truth;
    # Gen: Generated / Restored / Recovered images
    folder_GT = 'D:\TTST\Dataset/test\DIOR1000\GT/'   # GT 图像路径（不要包含中文）
    folder_test = 'D:\SR-Mamba\code/results\RGT\DIOR/19-34-37'   # 所要计算的结果路径
    writer = pd.ExcelWriter('DIOR/RGT-epoch500.xlsx')  # 关键2，创建名称为hhh的excel表格
    ...
    subfolder_GT_l = [folder_GT]  # DOTA DIOR计算时用，AID时注释

    # for each subfolder-video clip
    for subfolder in subfolder_GT_l:
        PSNR_all = []
        ...
        img_list = sorted(glob.glob(osp.join(subfolder, '*')))  # 真值图像路径列表
        ...
        for i, img_path in enumerate(img_list):
            ...
            mod_scale = 4
            width = int(np.floor(im_GT_in.shape[1] / mod_scale))
            height = int(np.floor(im_GT_in.shape[0] / mod_scale))
            # modcrop
            im_GT_in = im_GT_in[0:mod_scale * height, 0:mod_scale * width]
            im_Gen_in = im_Gen_in[0:mod_scale * height, 0:mod_scale * width]
            ...
            if crop:
                if im_GT_in.ndim == 3:
                    ...
                    cropped_GT = im_GT_in[crop_border:-crop_border, crop_border:-crop_border]
                    cropped_Gen = im_Gen_in[crop_border:-crop_border, crop_border:-crop_border]
                    cropped_GT = cv2.resize(cropped_GT, (cropped_Gen.shape[1], cropped_Gen.shape[0]),interpolation=cv2.INTER_NEAREST)
                    # print(cropped_GT.shape)
                    # print(cropped_Gen.shape)
                else:
            ...
            # PSNR = calculate_correlation_coefficient(cropped_GT * 255, cropped_Gen * 255)
            PSNR = calculate_psnr(cropped_GT * 255, cropped_Gen * 255)
            SSIM = calculate_ssim(cropped_GT * 255, cropped_Gen * 255)
            # SSIM = niqe(cropped_Gen * 255)

            # print('{:3d} - {:25}. \tPSNR: {:.6f} dB, \tSSIM: {:.6f}'.format(
            #     i + 1, base_name, PSNR, SSIM))
            logger.info('Folder {} {:3d} - {:25}. \tPSNR: {:.2f} dB, \tSSIM: {:.6f}'.format(subfolder_name,
                i + 1, base_name, PSNR, SSIM))
        ...
        logger.info('Folder {}-Average: PSNR: {:.2f} dB, SSIM: {:.6f}'.format(subfolder_name,
            sum(PSNR_all) / len(PSNR_all),
            sum(SSIM_all) / len(SSIM_all)))
        All_test_PSNR.append(sum(PSNR_all) / len(PSNR_all))
        All_test_SSIM.append(sum(SSIM_all) / len(SSIM_all))

    logger.info('All Folder Average: PSNR: {:.2f} dB, SSIM: {:.6f}'.format(sum(All_test_PSNR) / len(All_test_PSNR), sum(All_test_SSIM) / len(All_test_SSIM)))

    ## 将all——psnr存入excel
    data_df = pd.DataFrame(All_test_PSNR)  # 关键1，将ndarray格式转换为DataFrame
    data_df2 = pd.DataFrame(All_test_SSIM)  # 关键1，将ndarray格式转换为DataFrame
    data_df.to_excel(writer, 'PSNR', float_format='%.2f')  # 关键3，float_format 控制精度，将data_df写到hhh表格的第一页中。若多个文件，可以在page_2中写入
    data_df2.to_excel(writer, 'SSIM', float_format='%.4f')  # 关键3，float_format 控制精度，将data_df写到hhh表格的第一页中。若多个文件，可以在page_2中写入
    writer.close()
